...
版本：0.7.0（发布状态见 STATUS.md）。实现基于 RFC 2849；这是有明确边界的结构检查配置，不是完整目录 Schema 或服务器兼容认证。

0.7.0 新增显式 JSON 配置及每文件变更数量限制；配置报告使用格式版本 3，旧接口仍为 1/2。0.6.0 起提供完整审阅和差异分页，不提高输入上限。完整报告上限 32 MiB；安全写出仍重新解析并核对语义。
...
可显式排除属性描述，默认不排除；报告保留规范化排除列表和命中次数，不跳过输入错误、重复 DN 或整条增减。旧 API 和默认 CLI 报告保留前 200 项；新会话 API、CLI 分页/全量选项和工作台可查找全部已识别项。完整报告保留不完整状态，超过 32 MiB 明确失败；见 PAGINATION.md。

## 项目配置与数量限制

配置最多 64 KiB UTF-8。common 与当前模式段生效，未知字段/重复键/错误类型拒绝。变更记录、整条删除、无值 delete/replace 组数可分别限额；大于限额拦截，等于允许，0 禁止。batch 按文件计算，不提供合并限额。内容导出不能以变更限制冒充保护；不完整计数为已知下界，2 优先。完整定义见 [PROFILES.md](PROFILES.md)。
