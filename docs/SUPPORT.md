...
版本：0.1.0-dev.1。实现基于 RFC 2849；这是有明确边界的结构检查配置，不是完整目录 Schema 或服务器兼容认证。
...
| 核心目标平台 | 目前实际验证 MoonBit JS + Node.js 24；其他目标暂不声明已支持 |

## 结构边界

- 属性 `dn` 位于记录首行；后续出现 `dn` 被视为可能漏掉分隔空行并报错。名为 `control` 的普通内容属性可以解析；记录前部的 `changetype` 会按变更标记解释。
- 不检查完整 DN/RDN 语法、规范化、重复条目、父条目存在性、属性类型/Schema、必填属性、权限、控制语义、服务端插件或真实导入结果。
