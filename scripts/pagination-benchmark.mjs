...
if(process.argv[2]==='--worker') {
  ...
  for(let i=0;i<8;i++) {
    ...
    const config={before_sha256:sha(source),after_sha256:sha(after)};
    if(useProfile){
      const text=JSON.stringify({profile_version:1,review:{limits:{max_change_records:n-1}}});
      const encoded=Buffer.from(text).toString('base64');
      const p=JSON.parse(core.profile_validate(encoded));assert.equal(p.exit_code,0);
      Object.assign(config,{profile_encoded:encoded,profile_source_sha256:sha(text),profile_effective_sha256:sha(mode==='review'?p.canonical_review:p.canonical_compare)});
    }
    const s=core.paged_start(mode==='review'?'review':'compare',Buffer.from(source).toString('base64'),Buffer.from(after).toString('base64'),JSON.stringify(config));
    ...
    if(useProfile){assert.equal(first.report_schema_version,3);assert.equal(first.exit_code,1);}
